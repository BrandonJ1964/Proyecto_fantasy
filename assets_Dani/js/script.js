...
// Función para truncar títulos que son demasiado largos
function truncarTitulo(titulo, maxLongitud) {
  if (titulo.length > maxLongitud) {
      return titulo.substring(0, maxLongitud) + '...';
  }
  return titulo;
}
              ...
              <div class="movie-item p-2">
                  <div class="card" onclick='abrirDetalles(${JSON.stringify(pelicula)})'>
                      ...
                      <div class="card-body text-center">
                          <h5 class="card-title">${truncarTitulo(pelicula.titulo, 20)}</h5>
                      </div>
                  </div>
              </div>
          `;
